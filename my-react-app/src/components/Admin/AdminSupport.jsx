...
export function AdminSupport() {
  ...
  const handleViewFeed = (targetFeedId, targetTopic) => {
    navigate(`/admin/adminsupportchat?ticket=${targetFeedId}&topic=${targetTopic}`)
  }
  ...
  const handleProcessData = (targetData) => {
    ...
    return(<>
      ...
      <div className="spills "  onClick={() => tempHandleViewFeed.current(data._id, data.Topic) } title='View this feed'  >
  ...
  const fetchData = async () => {
    setIsLoading(true)
    let url = `${API_base_url}api/v1/supporttickets/open`
    try {
      const response = await fetch(url , {
        method: 'GET',
        headers: {
