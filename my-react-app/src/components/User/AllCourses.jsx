...
export function AllCourses() {
  ...
  const handleAdd = async (targetCourseId) => {
    try {
        const response = await fetch(`${API_base_url}api/v1/users/addcourse/${targetCourseId}`, {
            method: 'PATCH',
            headers: {
              // 'Content-Type': 'application/json',
              'authorization': `Bearer ${getStoredToken()}`
  ...
  tempHandleAdd.current = handleAdd
  useEffect(() => {
      ...
      return(
            <div className={`son3x myspans Overideflexdirection1ToRow`}  key={data._id}>
              ...
              <div className=" Overideflexdirection1ToCol"  title='View this user profile' >
                ...
                <br />
                <div className={`cls${data._id}`} role="button"  onClick={() => {if(window.confirm('Add this course?')){tempHandleAdd.current(data._id)} }} title='Add course' >
                { AddIcon } 
                </div>
              </div>
            </div>
