...
export function PrefferdCourses() {
  ...
  let tempHandleEnrollCourse  = useRef( )
  ...
  useEffect(() => {
    setPageTitle('PREFERRED COURSES')
    return () => {
    };
  }, [ setPageTitle ]);
  ...
  const handleEnrollCourse = (targetUserrId) => {
    navigate(`/user/enrollcourse?targetCourseId=${targetUserrId}`)
  }
  tempHandleEnrollCourse.current = handleEnrollCourse
      ...
      return(
            <div className={`son3x myspans Overideflexdirection1ToRow`}  key={data._id}>
              ...
              <div className=" Overideflexdirection1ToCol"  title='View this user profile' >
                <br />
                <div role="button" onClick={() => tempHandleEnrollCourse.current(data._id) } title='Pay now to enroll' >
                { PayIcon } 
                </div>
                <br />
                <div className={`cls${data._id}`} role="button"  onClick={() => {if(window.confirm('delete this course from your courses?')){tempHandleDelete.current(data._id)} }} title='delete from preferred course' >
                { DeleteIcon } 
                </div>
              </div>
            </div>
